...
module.exports = function (cb) {
  ...
  if (cluster.isMaster) {
    ...
    crier.registrySay(':vertical_traffic_light: Term Serialization MMS Containers starting.')
    ...
    var spawnWorker = () => {
      ...
      worker.on('message', function (msg) {
        ...
        if (msg.counter) {
          ...
          activeMessage = `MMS Containers Term Serialization: ${percent}% ${totalDone} Containers worked. Terms Processsed: ${totalAgentsAdded} Bots:${totalBots}`
    ...
    cluster.on('exit', (worker, code, signal) => {
      ...
      setTimeout(() => {
        if (Object.keys(cluster.workers).length === 0) {
          crier.registrySay(activeMessage)
          crier.registrySay(':checkered_flag: MMS Containers Term Serialization: Complete')
          if (cb) {
            setTimeout(() => {
              cb()
    ...
    process.on('message', (msg) => {
      ...
      if (msg.work) {
        ...
        db.returnCollectionRegistry('mmsContainers', (err, mmsContainers) => {
          ...
          _(mmsContainers.find({$and: [{mmsDb: {$gte: msg.work.start}}, {mmsDb: {$lt: msg.work.end}}]}, {'subjects': 1, 'mmsDb': 1}))
            .map((mmsContainer) => {
              ...
              // ++ the counter
              if (!mmsContainer.subjects) return ''
              if (mmsContainer.subjects.length === 0) return ''

              // the the subjects into the shape the methods expect
              mmsContainer.terms = mmsContainer.subjects.map((term) => {
              ...
              mmsContainer.terms = mmsContainer.terms.filter((term) => (term))
            ...
            .map((mmsContainer) => {
              ...
              var newTerms = []
              mmsContainer.terms.forEach((term) => {
                if (term.preconfigured && term.fast) {
                  ...
                  mmsContainer.splited = true
                } else {
                  // just a normal term add it to the pile
                  newTerms.push(term)
                }
              })
              mmsContainer.terms = newTerms
              return mmsContainer
            })
            ...
            .map((mmsContainer) => {
              ...
              mmsContainer.terms.forEach((term) => {
                ...
                term.source = {source: 'mmsContainer', id: mmsContainer._id}
