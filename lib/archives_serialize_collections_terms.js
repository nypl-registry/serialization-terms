        ...
        db.returnCollectionRegistry('archivesCollections', (err, archivesCollections) => {
          ...
          _(archivesCollections.find({$and: [{mssDb: {$gte: msg.work.start}}, {mssDb: {$lt: msg.work.end}}]}, {'subjects': 1, 'mssDb': 1}))
            ...
            .map((archivesCollections) => {
              ...
              //     var r = utils.buildAgentFromArchiveAgent(agent)
              //     updateAgents.push(r)
              //   }
              // })
            ...
            .compact()
            // .map(_.curry(utils.updateArchivesAgentsByViaf)) // add/update the data to the agents collection (it will filter out non viaf ones)
            // .nfcall([])
            // .series()
            // .map(_.curry(utils.updateArchivesAgentsByName)) // add/update the data to the agents collection
            // .nfcall([])
            // .series()
