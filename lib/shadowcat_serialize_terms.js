    ...
    var spawnWorker = () => {
      ...
      worker.on('message', function (msg) {
        ...
        if (msg.counter) {
          ...
          activeMessage = `Shadowcat Terms Serialization: ${percent}% ${totalDone} Bibs worked. Agents Added: ${totalTermsAdded} Bots:${totalBots}`
    ...
    process.on('message', (msg) => {
      ...
      if (msg.work) {
        ...
        db.returnCollectionShadowcat('bib', (err, bibs) => {
          ...
          // _(bibs.find({_id: 10000056}, {'sc:terms': 1, 'sc:research': 1}))
            ...
            .map((bib) => {
              ...
              bib.terms.forEach((term) => {
                ...
                // do not keep trying to push the same term if it happend recently
                if (termCache.indexOf(r.termControlled) > -1) {
                  return ''
                }
            ...
            .series()
            .map((termControlleds) => {
              termControlleds.forEach((termControlled) => termCache.push(termControlled))
              if (termCache.length > 10000) {
                for (var x = 0; x < termCache.length - 10000; x++) {
                  termCache.shift()
                }
              }

            // console.log(termCache.length)
            })
            .done((bib) => {
              console.log('Done')
              console.log('Done Working. #', cluster.worker.id)
