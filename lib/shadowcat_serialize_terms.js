    ...
    var spawnWorker = () => {
      ...
      worker.on('message', function (msg) {
        ...
        if (msg.counter) {
          ...
          process.stdout.write(clc.black.bgYellowBright(`Shadowcat Terms Serialization: ${Math.floor(totalDone / totalTodo * 100)}% ${++totalDone} Bots:${totalBots}`))
        }
        if (msg.restart) {
      ...
      if (msg.work) {
        ...
        db.returnCollectionShadowcat('bib', (err, bibs) => {
          ...
          _(bibs.find({$and: [{_id: {$gte: msg.work.start}}, {_id: {$lt: msg.work.end}}]}, {'sc:terms': 1, 'sc:research': 1}))
            ...
            .compact()
            .map(_.curry(utils.lookupAllTermsInFastByFast)) // fill in the VIAF data
            .nfcall([])
            .series()
            .map(_.curry(utils.lookupAllTermsInFastByPrefLabel)) // fill in the VIAF data
            .nfcall([])
            .series()
            .map(_.curry(utils.lookupAllTermsInFastByLcId)) // fill in the VIAF data
            .nfcall([])
            .series()
            // .map(_.curry(utils.lookupShadowcatTermsInFastByTerm)) // fill in the VIAF data
            // .nfcall([])
            // .series()
            .map((bib) => {
              console.log(bib)
