...
module.exports = function (cb) {
  ...
  if (cluster.isMaster) {
    ...
    // crier.registrySay(':vertical_traffic_light: Term Serialization - Starting Enumerate Agents.')
    ...
    var spawnWorker = () => {
      ...
      worker.on('message', function (msg) {
        ...
        if (msg.counter) {
          ...
          activeMessage = `Enumerate Agents: ${percent}% ${totalDone} Agents worked. Bots:${totalBots}`
    ...
    cluster.on('exit', (worker, code, signal) => {
      ...
      setTimeout(() => {
        if (Object.keys(cluster.workers).length === 0) {
          // crier.registrySay(activeMessage)
          // crier.registrySay(':checkered_flag: Enumerate Agents: Complete')
          if (cb) {
            setTimeout(() => {
              cb()
