...
module.exports = function (cb) {
  ...
  if (cluster.isMaster) {
    ...
    crier.registrySay(':vertical_traffic_light: Term Serialization MMS Collections starting.')
    ...
    var spawnWorker = () => {
      ...
      worker.on('message', function (msg) {
        ...
        if (msg.counter) {
          ...
          activeMessage = `MMS Collections Term Serialization: ${percent}% ${totalDone} Collections worked. Terms Processsed: ${totalAgentsAdded} Bots:${totalBots}`
    ...
    cluster.on('exit', (worker, code, signal) => {
      ...
      setTimeout(() => {
        if (Object.keys(cluster.workers).length === 0) {
          crier.registrySay(activeMessage)
          crier.registrySay(':checkered_flag: MMS Collections Term Serialization: Complete')
          if (cb) {
            setTimeout(() => {
              cb()
    ...
    process.on('message', (msg) => {
      ...
      if (msg.work) {
        ...
        db.returnCollectionRegistry('mmsCollections', (err, mmsCollections) => {
          ...
          _(mmsCollections.find({$and: [{mmsDb: {$gte: msg.work.start}}, {mmsDb: {$lt: msg.work.end}}]}, {'subjects': 1, 'mmsDb': 1}))
            .map((mmsCollection) => {
              ...
              // ++ the counter
              if (!mmsCollection.subjects) return ''
              if (mmsCollection.subjects.length === 0) return ''

              // the the subjects into the shape the methods expect
              mmsCollection.terms = mmsCollection.subjects.map((term) => {
              ...
              mmsCollection.terms = mmsCollection.terms.filter((term) => (term))
            ...
            .map((mmsCollection) => {
              ...
              var newTerms = []
              mmsCollection.terms.forEach((term) => {
                if (term.preconfigured && term.fast) {
                  ...
                  mmsCollection.splited = true
                } else {
                  // just a normal term add it to the pile
                  newTerms.push(term)
                }
              })
              mmsCollection.terms = newTerms
              return mmsCollection
            })
            ...
            .map((mmsCollection) => {
              ...
              mmsCollection.terms.forEach((term) => {
                ...
                term.source = {source: 'mmsCollection', id: mmsCollection._id}
