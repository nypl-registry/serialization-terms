...
exports.buildTerm = function (systemTerm) {
  ...
  if (systemTerm.term) {
    newTerm.termNormalized.forEach((norm) => {
      if (systemTerm.term.termNormalized.indexOf(norm) === -1) newTerm.hasNewLocal = true
    })
  }
...
/**
* Look up a TERM in VIAF, it might be name that got through
*
* @param  {string} term - the terms to search for
* @param  {function} cb - callback
*/
exports.returnViafByTerm = function (term, cb) {
  db.returnCollectionRegistry('viaf', function (err, viafLookup) {
    if (err) console.log(err)
    viafLookup.find({ normalized: normalize.normalizeAndDiacritics(term) }).toArray(function (err, records) {
      if (err) console.log(err)
      if (records.length > 0) {
        if (cb) cb(err, {id: term, data: true})
      } else {
        if (cb) cb(err, {id: term, data: false})
      }
    })
...
/**
* Lookup all terms in VIAF if we did not find it as a match in FAST yet, we want to filter out any agents that got in here
*
* @param  {object} termObject - object with terms array
* @param  {function} cb - callback
*/
exports.lookupAllTermsInViafByTerm = (termObject, cb) => {
  // builds a lookup of all the terms so we can save time if there are the same agent twice (contributor and subject for example)
  var lookup = {}
  ...
  _(termObject.terms)
    ...
    .compact()
    .map(_.curry(exports.returnViafByTerm))
    .nfcall([])
    .parallel(5)
    .map((results) => {
      lookup[results.id] = results.data
    ...
    .done(() => {
      // backfill in the data
      termObject.terms = termObject.terms.map((term) => {
        term.isAgent = lookup[term.nameLocal]
        return term
      })
      if (cb) cb(null, termObject)
    })
