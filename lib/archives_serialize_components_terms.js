...
module.exports = function (cb) {
  ...
  if (cluster.isMaster) {
    ...
    // crier.registrySay(':vertical_traffic_light: Terms Serialization Archives Components starting.')
    ...
    cluster.on('exit', (worker, code, signal) => {
      ...
      setTimeout(() => {
        if (Object.keys(cluster.workers).length === 0) {
          // crier.registrySay(activeMessage)
          // crier.registrySay(':checkered_flag: Archives Components Term Serialization: Complete')
          if (cb) {
            setTimeout(() => {
              cb()
